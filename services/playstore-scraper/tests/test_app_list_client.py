...
async def test_fetch_active_apps_requests_active_only():
    route = respx.get("http://app-list-api/apps", params={"active_only": "true"}).mock(
        return_value=httpx.Response(
            200,
            json=[{"id": 1, "package_name": "com.whatsapp", "name": "Whatsapp", "category": "messenger", "is_active": True}],
        )
    )
    ...
    assert route.called
    assert apps == [{"id": 1, "package_name": "com.whatsapp", "name": "Whatsapp", "category": "messenger", "is_active": True}]


@respx.mock
async def test_fetch_active_apps_raises_on_error_response():
    respx.get("http://app-list-api/apps", params={"active_only": "true"}).mock(return_value=httpx.Response(500))
